...
def plot_pileup(segmentManager, outName):
    ...
    fig = plt.figure(figsize=(10, 12), frameon=False, dpi=72) # plotHeight), frameon=False, dpi=96)
...
#         iter += 1
    ...
    plt.savefig(outName + '.svg', bbox_inches='tight', dpi=72)
    plt.close(fig)
...
def plot_indel_track(ax, yCoord, xOffset, segmentManager):
    ...
    for i, segment in enumerate(segmentManager.segments):
        ...
        for j, coord in enumerate(indelCoordinates):
            ...
            ax.add_patch(rect)
            xCoordLabel = xCoord + (float(rectLen) / float(2))
            ax.text(xCoordLabel, yCoord + 0.6, segment.indelSizes[j], ha='center', va='top', size=10)
