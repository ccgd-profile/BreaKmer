...
def plot_realignment_strands(ax, cSeq, seqYidx, xoffset):
    # Add realignment strand
    for segment in cSeq.segments:
        midDist = float(abs(segment.coords[1] - segment.coords[0])) / float(2)
        xcoord = xoffset + segment.coords[0] + midDist
        ax.text(xcoord, seqYidx+0.5, segment.alignStrand, ha='center', va='bottom', size=14, family='monospace')

def plot_contig_seq(ax, cSeq, contigSeq, seqYidx, xoffset):
    xinc = 1
    # Add 5' designation to contig sequence.
    add_seq_text(ax, 1, seqYidx, "5'")
    # Iterate over the nucleotides of the contig sequence.
    nucIter = 1
    for nuc in contigSeq:
...
def plot_pileup_seq(ax, cSeq, seqYidx, xoffset, orderedSeqs):
    yinc = 0.75
    xinc = 1
...
def plot_pileup(orderedSeqs, svRes, annot, outName):
    ...
    xoffset = 2
    xinc = 1
    plot_realignment_strands(ax, cSeq, seqYidx, xoffset)
    plot_contig_seq(ax, cSeq, svRes.contig, seqYidx, xoffset)
...
#        exonXoffset += segAnnot.segment.plotLen
    ...
    '''
    if annotBp1 and annotBp2:
        f1 = get_coding_features(annotBp1, gene1, strand1, 1, svRes.brkpts[0].split(':')[1])
        f2 = get_coding_features(annotBp2, gene2, strand2, 2, svRes.brkpts[1].split(':')[1])

        mapped_exons1, genomicEnd1 = map_coding_features(f1, strand1, 1, svRes.brkpts[0].split(':')[1], segLens[0])
        mapped_exons2, genomicEnd2 = map_coding_features(f2, strand2, 2, svRes.brkpts[1].split(':')[1], segLens[1])

        # Plot the exons as rectangles.

        # First segment exons
        for exCoords in mapped_exons1:
            rect = patches.Rectangle((xoffset + exCoords[0], yidx), exCoords[1]-exCoords[0], 2, color='green')
            ax.add_patch(rect)
            ax.text(xoffset + exCoords[0], yidx+2, exCoords[2], ha='left', va='bottom', size=10, rotation=60)

        # Second segment exons
        for exCoords in mapped_exons2:
            rect = patches.Rectangle((xoffset + segLens[0] - sum(overlaps) + exCoords[0], yidx - 0.2), exCoords[1]-exCoords[0], 2, color='orange')
            ax.add_patch(rect)
            ax.text(xoffset + segLens[0] - sum(overlaps) + exCoords[0], yidx-0.2 + 2, exCoords[2], ha='left', va='bottom', size=10, rotation=60)

        # Gene labels
        ax.text(segLens[0]/2, yidx + 5, gene1, ha='center', va='center', size=16, style='italic')
        ax.text(segLens[0] + segLens[1]/2, yidx + 5, gene2, ha='center', va='center', size=16, style='italic')

    # Gene coordinates
    # Segment1 left
    ax.text(xoffset, yidx - 0.75, chr1+':'+str(genomicEnd1), ha='left', va='center', size=14)
    # Segment1 right
    ax.text(segLens[0] + xoffset - 1, yidx - 0.75, chr1+':'+bp1, ha='right', va='center', size=14)

    # Segment2 left
    ax.text(xoffset + segLens[0]+1, yidx - 0.2 - 0.75, chr2+':'+str(int(bp2)+overlaps[0]), ha='left', va='center', size=14)
    # Segment2 right
    ax.text(segLens[1]+segLens[0], yidx - 0.2 - 0.75, chr2+':'+str(genomicEnd2), ha='right', va='center', size=14)
    '''

    '''
    annotBp1, gene1, strand1, trxId1 = annot.get_trx_exons(svRes.brkpts[0])
    annotBp2, gene2, strand2, trxId2 = annot.get_trx_exons(svRes.brkpts[1])

    yidx = (len(orderedSeqs)+1)*0.75 + 1.5 + 5
    if annotBp1 and annotBp2:
        f1 = get_coding_features(annotBp1, gene1, strand1, 1, svRes.brkpts[0].split(':')[1])
        f2 = get_coding_features(annotBp2, gene2, strand2, 2, svRes.brkpts[1].split(':')[1])
    ...
    # Solid line for gene models segment1
    plt.plot((xoffset, segLens[0] + xoffset), (yidx, yidx), color='green', linewidth=2)
    # Solid line for gene models segment2
    plt.plot((segLens[0] - sum(overlaps), contigLen + sum(overlaps) - xoffset), (yidx-0.2, yidx-0.2), color='orange', linewidth=2)

    # Vertical line for breakpoint.
    for iter, brk in enumerate(breakIdx):
        xidx = brk + xoffset + iter
        plt.plot((xidx, xidx), (0, yidx), color='red', linewidth=2)
    chr1, bp1 = svRes.brkpts[0].split(':')
    chr2, bp2 = svRes.brkpts[1].split(':')

    # Nucleotide coordinates
    # Segment1 left
    ax.text(xoffset, 0.5, chr1+':'+str(int(bp1)-segLens[0]), ha='left', va='center', size=14)
    # Segment1 right
    ax.text(segLens[0] + xoffset - 1, 0.5, chr1+':'+bp1, ha='right', va='center', size=14)

    # Segment2 left
    ax.text(xoffset + segLens[0]+1, 0.5, chr2+':'+str(int(bp2)+overlaps[0]), ha='left', va='center', size=14)
    # Segment2 right
    ax.text(segLens[1]+segLens[0], 0.5, chr2+':'+str(int(bp2)+segLens[1]), ha='right', va='center', size=14)
...
def pile_reads(reads, contigSeq):
    ...
    for read in reads:
        ...
        add = True
        if idx == -1:
            aln1 = olc.nw(contigSeq, read.seq)
            aln2 = olc.nw(read.seq, contigSeq)
#            print 'aln1', aln1
#            print 'aln2', aln2
...
if __name__ == "__main__":
    ...
    print resultFn
    print gene_pair
    resPath = os.path.split(resultFn)[0]
    contigIds = []
    for line in open(resultFn, 'r'):
        line = line.strip()
        linesplit = line.split('\t')
        gp = linesplit[0].replace('-TRL','')
        genes = gp.split(',')
        genes.sort()
        genestr = ':'.join(genes)
        if ggstr == genestr:
            contigId = linesplit[11]
